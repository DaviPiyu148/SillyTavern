...
- **REST API & World Gating**: 39 endpoints under `/api/living-world/worlds/:worldLwsId/*`. Gated API behavior returns `404 {"error": "World not found"}` for any descendant route when the parent world is soft-deleted, leaving underlying DB rows physically intact.
...
Phase 9 establishes the dynamic living world layer: location environments, location operational states, world-authored ambient archetypes, multi-tier population classification (Core, Supporting, Ambient), deterministic ephemeral population generation with collision-proof transient IDs, a dynamic entity promotion pipeline, sensory clarity scoring, and pure in-memory replay across all 20 simulation runtime tables/structures in SQLite under migration `009_environment_and_population` (`PRAGMA user_version = 9`).

#### 1. Implemented Phase 9 Scope
- **Schema & Migrations (Migration 009)**:
  - `lws_location_environments`: Environmental conditions per location (`weather`, `temperature_baseline`, `temperature_celsius`, `temperature_override`, `lighting_level`, `lighting_override`, `noise_level`, `is_indoor`, `air_quality`, `hazards`, `last_evaluated_fictional_time`).
  - `lws_location_operational_states`: Operational statuses (`access_status`, `access_override`, `operating_hours`, `crowd_density`, `ambient_capacity`).
  - `lws_ambient_archetypes`: World-level authored templates for ambient generation (`archetype_key`, `entity_kind`, `role_title`, `name_pool`, `description_template`, `default_activities`, `location_tags`, `time_windows`, `weather_compat`, `spawn_weight`, `max_concurrent_instances`).
  - `lws_simulation_character_tiers`: Character classification (`tier` ['core', 'supporting'], `cognitive_budget` ['full', 'lightweight'], `is_promoted`).
  - `lws_promoted_entity_records`: Immutable causal promotion ledger (`simulation_character_id`, `source_archetype_key`, `source_transient_id`, `origin_location_id`, `promotion_reason`, `causal_event_id`, `promoted_to_tier`, `fictional_time`).
- **Database Boundary Hardening (Cumulative 85 Triggers)**:
  - Exactly 15 Phase 9 triggers enforcing simulation/location immutability, duplicate location assignment prevention, prohibiting direct physical DELETE across all 5 tables, and enforcing immutable append-only semantics for promoted entity records and operational state history.
- **Indexes (Cumulative 51 Indexes across Phase 4–9 Tables)**:
  - Exactly 10 Phase 9 indexes optimizing location environment/operational state lookups, world-scoped archetype retrieval, simulation character tier lookups, and promoted entity provenance tracking.
- **Environment & Population Modules**:
  ...
  - **Procedural Ambient Generation**: Seeded pseudo-random generation producing deterministic transient IDs (`amb:<sim>:<loc>:<bucket>:<slotIndex>`) without persisting rows in SQLite (with defensive 6-part parsing support).
  - **Dynamic Entity Promotion**: 4-stage pipeline verifying transient ID existence, creating authored character rows, committing atomic `CHARACTER_JOIN`, and logging immutable promotion provenance in `lws_promoted_entity_records`.
...
- **Pure Replay & Field-Level Parity**:
  ...
  - `verifySimulationParity` proves 100% field-level parity across all 20 simulation runtime tables/structures across 21 verification comparison steps.
- **REST API (Tri-Tier Routes)**:
  - Mounts 12 Phase 9 endpoints across Tier 1 (Observer: environment, operational states, ambient population estimate, promoted entities), Tier 2 (Subjective character: perspective environmental sensing, character tier), and Tier 3 (Director: profile overrides, state transitions, archetype management, entity promotion).

### Phase 10 Implementation Details and Scope Distinction

Phase 10 establishes the prompt assembly, perspective-isolated context construction, token budgeting, structured output parsing, generation pipeline, and REST transport layers connecting SillyTavern's generation pipeline with the Living World Simulator runtime under commit `c219b390a` (schema remains `PRAGMA user_version = 9`; zero database migrations required).

#### 1. Implemented Phase 10 Scope
- **Prompt & Context Modules (`src/living-world/prompt/`)**:
  - `common.js`: Canonical taxonomies for 4 generation modes (`character_dialogue`, `character_decision`, `world_narration`, `director_query`), 12 dynamic prompt layers (`lws_system_contract`, `simulation_state`, `world_premise_rules`, `style_and_author_instructions`, `character_authored_profile`, `character_mind_state`, `permitted_relationships`, `permitted_perception_scene`, `relevant_memories_and_knowledge`, `recent_causal_events`, `director_instruction_or_user_prompt`, `output_contract`), 4 output contracts (`dual_block`, `structured_proposal`, `narrative_prose`, `director_report`), priority weights ($20..100$), and default contract templates.
  ...
  - `token-budget.js`: Deterministic token estimation (`estimateTokens`) and priority-based graceful layer trimming (`allocateTokenBudget`), preserving fixed layers (`lws_system_contract`, `output_contract`, `director_instruction_or_user_prompt`) while gracefully truncating/pruning elastic layers (`recent_causal_events`, `relevant_memories_and_knowledge`, `permitted_relationships`) when over budget.
  ...
  - `generation.js`: Turn orchestrator (`generateSimulationTurn`) executing context assembly $\to$ token budgeting $\to$ model transport (ST provider/driver callback) $\to$ proposal parsing $\to$ 2-transaction savepoint execution (`executeNarrativeTurn`).
- **Domain & Authority Enhancements**:
...
- **REST Transport (`src/endpoints/living-world.js`)**:
  - `POST /api/living-world/simulations/:simLwsId/prompt-context/build`: Inspects layered prompt context and token budget allocations.
  - `POST /api/living-world/simulations/:simLwsId/generate`: Executes full simulation turn generation and proposal execution under savepoint isolation.
- **Dedicated Test Suites (`tests/living-world/`)**:
  - 6 dedicated Phase 10 test suites passing (22/22 tests): `lws-prompt-context-builder.test.js`, `lws-prompt-knowledge-isolation.test.js`, `lws-token-budgeting.test.js`, `lws-output-parser.test.js`, `lws-generation-pipeline.test.js`, `lws-prompt-api.test.js`.
  - Cumulative Living World subsystem: 69 suites, 459/459 tests passing. Full repository: 88 suites, 870/870 tests passing.
- **Architecture Decision Record**:
  - Authored `docs/living-world/decisions/ADR-018-prompt-context-and-generation-integration.md`.

### Phase 11 Implementation Details and Scope Distinction

Phase 11 establishes the complete import, normalization, provenance, conflict resolution, preview/commit token binding, and canonical world manifest exchange pipeline in the Living World Simulator subsystem (schema remains `PRAGMA user_version = 9`; zero database migrations required).

#### 1. Implemented Phase 11 Scope
- **Import & Normalization Modules (`src/living-world/import/`)**:
  - `common.js`: Unicode NFC normalization (`normalizeUnicode`), prototype pollution sanitizer (`sanitizePrototype`), RFC 8785 canonical JSON serializer (`canonicalizeJson`), 10-table canonical preview state hash (`computePreviewStateHash`), HMAC-SHA256 preview token generator and verifier (`generatePreviewToken`, `verifyPreviewToken`), provenance record builder (`createProvenance`).
...
- **REST Transport (`src/endpoints/living-world.js`)**:
  - `POST /api/living-world/import/character/preview`: Character card preview with advisory token and conflict report.
  - `POST /api/living-world/worlds/:worldLwsId/import/character`: Character card atomic commit into target world.
  - `POST /api/living-world/import/worldinfo/preview`: Lorebook preview with structural classification and ambiguity flags.
  - `POST /api/living-world/worlds/:worldLwsId/import/worldinfo`: Lorebook atomic commit into target world.
  - `POST /api/living-world/import/freeform/preview`: Freeform outline preview and candidate generation.
  - `POST /api/living-world/worlds/:worldLwsId/import/freeform`: Freeform candidate commit into target world.
  - `POST /api/living-world/import/manifest/preview`: Canonical World Manifest preview and validation.
  - `POST /api/living-world/import/manifest/commit`: Canonical World Manifest atomic commit creating full world hierarchy.
  - `GET /api/living-world/worlds/:worldLwsId/export/manifest`: Symmetric round-trip export of full world manifest.
- **Dedicated Test Suites (`tests/living-world/`)**:
  - 8 dedicated Phase 11 test suites passing (74/74 tests): `lws-provenance.test.js` (16), `lws-card-importer.test.js` (10), `lws-worldinfo-importer.test.js` (9), `lws-manifest-importer.test.js` (6), `lws-ai-normalizer.test.js` (7), `lws-conflicts.test.js` (5), `lws-authoring-bundle.test.js` (7), `lws-import-api.test.js` (14).
  - Cumulative Living World subsystem: 78 suites, 537/537 tests passing.
- **Architecture Decision Record**:
